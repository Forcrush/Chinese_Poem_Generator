...
# 检查诗文格式
def standard_poem(poem):
    seg = [i for i in re.split('\，|\。|\？|\\n', poem) if i]
    # print(seg, len(seg))
    if len(seg) % 2 != 0:
        # print('诗句不为偶数')
        return False
    for i in range(0, len(seg), 2):
        if len(seg[i]) == 5 or len(seg[i]) == 7:
            if len(seg[i]) == len(seg[i+1]):
                return True
            else:
                # print('诗句每联字数不同')
                return False
...
def sample(data, model, head=''):
    ...
    with tf.Session() as sess:
        ...
        if head:
            ...
            [probs, state] = sess.run([model.probs, model.final_state], feed_dict)
            word = to_word(probs[-1])
            while word != END_CHAR and legal_step(step):
                step += 1
                poem += word
                x = np.zeros((1, 1))
                x[0, 0] = data.char2id(word)
...
def begin_gen(head=''):
    ...
    result = sample(data, model, head=head).replace('。', '。\n')
    if standard_poem(result):
        return result
    else:
        return "None"
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='sample', choices=['sample', 'train'],
                        help=u'usage: train or sample, sample is default')
    parser.add_argument('--head', type=str, default='上善若水',
                        help='生成藏头诗')
